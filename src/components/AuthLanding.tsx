import React, { useState } from "react";
import {
  Award,
  UploadCloud,
  FileCheck,
  Sparkles,
  Shield,
  Lock,
  Cpu,
  Database,
  ArrowRight,
  AlertCircle,
  FileText,
  CheckCircle2,
} from "lucide-react";

interface AuthLandingProps {
  onSignIn: () => Promise<void>;
  isLoading: boolean;
}

export const AuthLanding: React.FC<AuthLandingProps> = ({ onSignIn, isLoading }) => {
  const [authError, setAuthError] = useState<string | null>(null);

  const handleSignInClick = async () => {
    try {
      setAuthError(null);
      await onSignIn();
    } catch (err: any) {
      if (err?.code === "auth/popup-closed-by-user") {
        setAuthError("Sign-in window was closed before completion. Please try again.");
      } else if (err?.code === "auth/popup-blocked") {
        setAuthError("Pop-up was blocked by browser. Please allow pop-ups for this domain to sign in.");
      } else {
        setAuthError(err?.message || "Failed to authenticate with Google. Please try again.");
      }
    }
  };

  return (
    <div
      id="landing-container"
      className="min-h-[calc(100vh-61px)] bg-[#F8F6F2] text-[#2C2C2C] flex flex-col justify-center items-center px-4 py-8 sm:px-6 lg:px-8"
    >
      <div className="w-full max-w-3xl mx-auto space-y-6">
        {/* Welcome Banner Card */}
        <div className="rounded-2xl bg-white border border-[#E8E4DF] shadow-[0_2px_12px_rgba(44,44,44,0.04)] overflow-hidden">
          {/* Header Banner */}
          <div className="p-6 sm:p-8 bg-[#FAF8F5] border-b border-[#F0ECE6] text-center space-y-3">
            <div className="inline-flex items-center justify-center p-3.5 rounded-2xl bg-[#F4EFEB] border border-[#E8E4DF] text-[#8B735B] shadow-2xs">
              <Award className="w-8 h-8" />
            </div>

            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center justify-center gap-2">
                <h1 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-[#2C2C2C]">
                  Academic Vault AI
                </h1>
                <span className="px-2.5 py-0.5 rounded-full bg-[#8B735B]/15 text-[#755F4A] text-xs font-medium font-sans">
                  Gemini 3.6 Flash &bull; Cloud Firestore
                </span>
              </div>
              <p className="text-sm sm:text-base text-[#736E68] max-w-xl mx-auto font-sans leading-relaxed">
                An intelligent academic credential archive and reflective advisor. Securely digitize marksheets, degrees, and certificates with structured metadata extraction, and consult your records with citation-grounded intelligence.
              </p>
            </div>
          </div>

          {/* Feature Overview: How to Use Academic Vault AI */}
          <div className="p-6 sm:p-8 space-y-6">
            <div>
              <h2 className="text-xs font-semibold text-[#2C2C2C] uppercase tracking-wider mb-3">
                How Academic Vault AI Works
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-left">
                {/* Step 1 */}
                <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#8B735B] text-white text-[11px] font-bold flex items-center justify-center font-mono shrink-0">
                      1
                    </span>
                    <span className="text-xs font-serif font-semibold text-[#2C2C2C]">
                      Upload Credentials
                    </span>
                  </div>
                  <p className="text-[11px] text-[#736E68] leading-relaxed">
                    Attach marksheets, degrees, or transcripts in PDF or image format via drag-and-drop.
                  </p>
                </div>

                {/* Step 2 */}
                <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#8B735B] text-white text-[11px] font-bold flex items-center justify-center font-mono shrink-0">
                      2
                    </span>
                    <span className="text-xs font-serif font-semibold text-[#2C2C2C]">
                      Review &amp; Archive
                    </span>
                  </div>
                  <p className="text-[11px] text-[#736E68] leading-relaxed">
                    Inspect extracted GPA, coursework, and honors in the verification modal before saving.
                  </p>
                </div>

                {/* Step 3 */}
                <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#8B735B] text-white text-[11px] font-bold flex items-center justify-center font-mono shrink-0">
                      3
                    </span>
                    <span className="text-xs font-serif font-semibold text-[#2C2C2C]">
                      Grounded Inquiries
                    </span>
                  </div>
                  <p className="text-[11px] text-[#736E68] leading-relaxed">
                    Ask questions about coursework strengths or GPA. Gemini answers with clickable citation badges.
                  </p>
                </div>

                {/* Step 4 */}
                <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#8B735B] text-white text-[11px] font-bold flex items-center justify-center font-mono shrink-0">
                      4
                    </span>
                    <span className="text-xs font-serif font-semibold text-[#2C2C2C]">
                      Summarize &amp; Export
                    </span>
                  </div>
                  <p className="text-[11px] text-[#736E68] leading-relaxed">
                    Generate structured takeaways, sentiment assessments, and export complete records to Markdown.
                  </p>
                </div>
              </div>
            </div>

            {/* Architecture Highlights */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left pt-1">
              <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1">
                <div className="flex items-center gap-2 text-[#2C2C2C] font-serif font-semibold text-xs">
                  <Shield className="w-4 h-4 text-[#8B735B]" />
                  <span>Owner-Isolated Security</span>
                </div>
                <p className="text-[11px] text-[#736E68] leading-relaxed">
                  Firestore rules enforce strict isolation to <code className="text-[10px] bg-white border border-[#E8E4DF] px-1 py-0.5 rounded font-mono text-[#755F4A]">/users/&#123;uid&#125;/documents</code>.
                </p>
              </div>

              <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1">
                <div className="flex items-center gap-2 text-[#2C2C2C] font-serif font-semibold text-xs">
                  <Cpu className="w-4 h-4 text-[#8B735B]" />
                  <span>Multimodal Extraction</span>
                </div>
                <p className="text-[11px] text-[#736E68] leading-relaxed">
                  Structured JSON schemas strictly validate marks, GPAs, and course grades via Gemini 3.6 Flash.
                </p>
              </div>

              <div className="p-3.5 rounded-xl bg-[#FAF8F5] border border-[#E8E4DF] space-y-1">
                <div className="flex items-center gap-2 text-[#2C2C2C] font-serif font-semibold text-xs">
                  <Database className="w-4 h-4 text-[#8B735B]" />
                  <span>Durable Vault Persistence</span>
                </div>
                <p className="text-[11px] text-[#736E68] leading-relaxed">
                  Zero-crash payload sanitation ensures credentials and reflections are permanently synced.
                </p>
              </div>
            </div>

            {/* Auth Error Banner if any */}
            {authError && (
              <div
                id="auth-error-banner"
                className="p-3.5 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm flex items-start gap-2.5 text-left shadow-2xs"
              >
                <AlertCircle className="w-5 h-5 text-red-600 shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="font-medium text-xs">Sign-In Notice</p>
                  <p className="text-xs text-red-600 mt-0.5">{authError}</p>
                </div>
              </div>
            )}

            {/* Google Sign-in Call to Action */}
            <div className="pt-2 flex flex-col items-center gap-3 border-t border-[#F0ECE6]">
              <button
                id="google-signin-btn"
                onClick={handleSignInClick}
                disabled={isLoading}
                className="w-full sm:w-auto min-w-[280px] inline-flex items-center justify-center gap-3 px-6 py-3.5 rounded-xl text-sm font-medium bg-[#2C2C2C] text-[#F8F6F2] hover:bg-[#1A1A1A] active:scale-[0.99] transition shadow-xs hover:shadow-sm disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer border border-[#1A1A1A]"
              >
                {isLoading ? (
                  <>
                    <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    <span>Authenticating with Google...</span>
                  </>
                ) : (
                  <>
                    <svg className="w-4 h-4" viewBox="0 0 24 24">
                      <path
                        fill="#4285F4"
                        d="M23.745 12.27c0-.7-.06-1.4-.19-2.07H12v4.51h6.6c-.29 1.52-1.14 2.82-2.4 3.68v3.05h3.88c2.27-2.09 3.665-5.17 3.665-9.17z"
                      />
                      <path
                        fill="#34A853"
                        d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.88-3.05c-1.08.72-2.45 1.16-4.05 1.16-3.12 0-5.77-2.1-6.72-4.93H1.26v3.15C3.26 21.36 7.36 24 12 24z"
                      />
                      <path
                        fill="#FBBC05"
                        d="M5.28 14.27c-.25-.72-.38-1.49-.38-2.27s.13-1.55.38-2.27V6.58H1.26C.46 8.16 0 9.94 0 12s.46 3.84 1.26 5.42l4.02-3.15z"
                      />
                      <path
                        fill="#EA4335"
                        d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.36 0 3.26 2.64 1.26 6.58l4.02 3.15c.95-2.83 3.6-4.98 6.72-4.98z"
                      />
                    </svg>
                    <span>Sign in with Google to Enter Vault</span>
                    <ArrowRight className="w-4 h-4 text-[#A39D95] ml-1" />
                  </>
                )}
              </button>

              <p className="text-xs text-[#736E68] flex items-center gap-1.5">
                <Lock className="w-3.5 h-3.5 text-[#8B735B]" />
                <span>Federated Google Sign-In via Firebase Auth. No passwords stored.</span>
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
